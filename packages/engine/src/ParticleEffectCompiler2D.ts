...
void main() {
  ...
  if (id < uCapacity && readCommand(id, commandA, commandB, commandC, commandD, relative)) {
    ...
    else if (shape == 9) { offset=vec2((randomA-.5)*source.x*2.0,0.0); angle=commandC.x+(randomB-.5)*commandC.y; }
    ...
    else if (shape == 11) { offset=vec2((randomA-.5)*source.x*2.0,(randomB-.5)*source.y*2.0); }
...
void main() {
  ...
  if(id<uCapacity && claim<12582912.0){
    ...
    int child=-1;float lifetime=0.0,inheritance=0.0,powerScale=0.0,spread=6.2831853,basePower=24.0,speedReference=1.0,lifetimeVariability=0.0,powerVariability=0.0,impactPowerScale=0.0,powerScaleVariability=0.0;
    ${eventBranches}
...
void main() {
  ...
  float continuity=clamp(uStreakScale,0.0,4.0);
  ...
  // Continuity bridges frame displacement only when requested. It never
  // forces a long streak when both authored length and continuity are zero.
  float continuityWeight=smoothstep(0.0,0.18,continuity);
  float coverageMultiplier=continuityWeight*(0.82+continuity*.78);
  float coverageLength=length(b.xy)*clamp(uFrameDelta,.001,.1)*coverageMultiplier;
...
void main() {
  ...
  float streakWidth=1.0-smoothstep(0.52,1.0,abs(vStreakUv.y));
  // Keep a small continuity-weighted seam so adjacent temporal samples join
  // without turning the complete streak into a uniformly bright beam.
  float streakJoin=0.18*smoothstep(0.0,0.3,uStreakScale);
  float streakTail=mix(streakJoin,1.0,pow(max(0.0,1.0-vStreakUv.x),1.35));
  float coverage = vStreak == 1 ? streakWidth*streakTail : 1.0 - smoothstep(uLayerKind==2?0.0:0.72, 1.0, radialDistance);
...
fn simulate(@builtin(global_invocation_id) gid: vec3<u32>) {
  ...
  if (frame.commandCount > 0u) {
    ...
    if (low > 0u) {
      ...
      else if (shape == 9u) { offsetPosition=vec2<f32>((randomA-0.5)*source.x*2.0,0.0); angle=c.x+(randomB-0.5)*c.y; }
      ...
      else if (shape == 11u) { offsetPosition=vec2<f32>((randomA-0.5)*source.x*2.0,(randomB-0.5)*source.y*2.0); }
