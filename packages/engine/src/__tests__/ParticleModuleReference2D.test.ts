...
describe('ParticleModuleReference2D',()=>{
  ...
  it('accumulates multiple attractors like the GPU field loop',()=>{const state={x:0,y:0,vx:0,vy:0,age:0,lifetime:2,rotation:0,angularVelocity:0};integrateParticleReference2D(state,{gravity:0,drag:0,radialAcceleration:1,tangentialAcceleration:0},1,[{x:10,y:0,strength:2},{x:0,y:10,strength:3}]);expect(state.vx).toBeCloseTo(2);expect(state.vy).toBeCloseTo(3);expect(state.x).toBeCloseTo(2);expect(state.y).toBeCloseTo(3);});
  ...
  it('samples every built-in distribution without non-finite output',()=>{for(const shape of ['point','disc','line','cone','arc','ring','radial','spiral','pinwheel','shower','annulus','rectangle'] as const)expect(Object.values(sampleParticleSpawnReference2D(shape,2,8,.25,.75,20,10,Math.PI,0,1)).every(Number.isFinite)).toBe(true);});
