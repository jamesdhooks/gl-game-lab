...
export function integrateParticleReference2D(state: ParticleReferenceState2D, motion: ParticleMotionProfile2D, delta: number, attractor?: ParticleAttractor2D): void {
  ...
  if (attractor && ((motion.radialAcceleration ?? 0) !== 0 || (motion.tangentialAcceleration ?? 0) !== 0 || (attractor.velocityCoupling ?? 0) !== 0)) {
    ...
    const radius=attractor.radius??0;if(radius>0&&rawLength>=radius){applyMotionTail(state,motion,delta);return;}
    const envelope=forceEnvelope(attractor,rawLength),mode=attractor.falloff??motion.radialFalloff,falloff=mode==='inverse-square'?1/(length*length):mode==='inverse'?1/length:1;
...
function applyMotionTail(state: ParticleReferenceState2D,motion:ParticleMotionProfile2D,delta:number):void{const damping=Math.exp(-Math.max(0,motion.drag)*delta);state.vx*=damping;state.vy*=damping;const speed=Math.hypot(state.vx,state.vy),maxSpeed=motion.maxSpeed??0;if(maxSpeed>0&&speed>maxSpeed){state.vx*=maxSpeed/speed;state.vy*=maxSpeed/speed;}state.rotation+=state.angularVelocity*delta;state.x+=state.vx*delta;state.y+=state.vy*delta;}
function forceEnvelope(attractor:ParticleAttractor2D,distance:number):number{const radius=attractor.radius??0;if(radius<=0||attractor.envelope==='none')return 1;const t=Math.max(0,Math.min(1,1-distance/radius));return attractor.envelope==='smooth'?t*t*(3-2*t):t;}

export function applyParticleDomainReference2D(state:ParticleReferenceState2D,domain:ParticleDomain2D):void{if(domain.behavior==='none')return;const damping=domain.damping??1,margin=domain.margin??0;if(domain.shape==='circle'){const dx=state.x-domain.center[0],dy=state.y-domain.center[1],distance=Math.hypot(dx,dy),radius=(domain.radius??0)+margin;if(distance<=radius)return;const nx=distance>1e-6?dx/distance:1,ny=distance>1e-6?dy/distance:0;if(domain.behavior==='kill'){state.age=state.lifetime;return;}if(domain.behavior==='wrap'){state.x=domain.center[0]-nx*(domain.radius??0);state.y=domain.center[1]-ny*(domain.radius??0);state.vx*=damping;state.vy*=damping;return;}const dot=state.vx*nx+state.vy*ny;state.x=domain.center[0]+nx*(domain.radius??0);state.y=domain.center[1]+ny*(domain.radius??0);state.vx=(state.vx-2*nx*dot)*damping;state.vy=(state.vy-2*ny*dot)*damping;return;}const extents=domain.halfExtents??[0,0],minX=domain.center[0]-extents[0]-margin,maxX=domain.center[0]+extents[0]+margin,minY=domain.center[1]-extents[1]-margin,maxY=domain.center[1]+extents[1]+margin;if(state.x>=minX&&state.x<=maxX&&state.y>=minY&&state.y<=maxY)return;if(domain.behavior==='kill'){state.age=state.lifetime;return;}if(domain.behavior==='wrap'){if(state.x<minX)state.x=maxX;else if(state.x>maxX)state.x=minX;if(state.y<minY)state.y=maxY;else if(state.y>maxY)state.y=minY;state.vx*=damping;state.vy*=damping;return;}if(state.x<minX||state.x>maxX){state.x=Math.max(minX,Math.min(maxX,state.x));state.vx=-state.vx*damping;}if(state.y<minY||state.y>maxY){state.y=Math.max(minY,Math.min(maxY,state.y));state.vy=-state.vy*damping;}}
...
export function sampleParticleSpawnReference2D(shape: ParticleSpawnShape2D, index: number, count: number, randomA: number, randomB: number, radius: number, length: number, arc: number, direction: number, spread: number): ParticleReferenceSpawn2D {
  ...
  else if(shape==='annulus'){const a=randomA*Math.PI*2,r=Math.sqrt(Math.max(0,length*length+(radius*radius-length*length)*randomB));x=Math.cos(a)*r;y=Math.sin(a)*r;angle=a;}
