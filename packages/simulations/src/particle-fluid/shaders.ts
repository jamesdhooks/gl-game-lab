...
uniform float uDt,uParticleDrag,uSimulationScale;
void main(){
  ...
  vec2 flow=texture(uFlowField,clamp(p.xy,vec2(.001),vec2(.999))).xy/max(.0001,uSimulationScale);
...
vec2 toSimulation(vec2 uv){vec2 clip=uv*2.-1.;return vec2(clip.x*uAspectRatio*uSimulationScale,clip.y*uSimulationScale);}
void main(){
  int id=gl_VertexID;ivec2 cell=ivec2(id%uStateSize.x,id/uStateSize.x);
  vec4 p=texelFetch(uPositionState,cell,0),v=texelFetch(uVelocityState,cell,0);
  vec2 clip=p.xy*2.-1.;
  float speed=length(v.xy)*2.,x=clamp(speed*uSpeedColorScale,0.,1.);
  vec3 color=mix(uSlowColor,uFastColor,x)+uHotColor*x*x*x*.1;
  float pulse=0.;
  if(uEnhanced>.5&&uPulseParams.y>.0001){
    vec2 distanceAndFraction=distanceToSegment(toSimulation(p.xy),toSimulation(uPulseSegment.xy),toSimulation(uPulseSegment.zw));
