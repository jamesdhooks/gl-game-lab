...
import { createCompiledOrbitalShrapnelPlugin } from "./CompiledOrbitalShrapnelPlugin.js";
...
export const orbitalShrapnelDefinition: ExperienceDefinition = {
  ...
  physics: {
    ...
    reusableFor: ["high-count GPU particle rendering", "gravity-well simulations", "trail-field compositing", "orbital debris effects"],
    caveats: ["Space Debris keeps a custom orbital force model on the shared GPU state pipeline."],
  },
  createPlugins: (options = {}) => [createCompiledOrbitalShrapnelPlugin(createOrbitalShrapnelConfig(options.settings), options)],
};
