...
export const ORBITAL_SHRAPNEL_PARTICLE_EFFECT: ParticleEffectDefinition2D = validateParticleEffectDefinition2D({
  id: 'orbital-shrapnel',
  capacity: { min: 64 * 64, default: 256 * 256, max: 2048 * 2048, previewMax: 128 * 128, commandCapacity: 64 },
  archetypes: [
    {
...
export const ORBITAL_SHRAPNEL_PARTICLE_GRAPH = defineParticleEffect2D({
  ...
  id: 'orbital-shrapnel',
  parameters: [
    { id: 'planet-gravity', kind: 'number', defaultValue: 1850, min: 0, max: 3200 },
  ...
  graph: {
    root: particleGraph2D.parallel(
      particleGraph2D.gate({ kind: 'signal', signal: 'add-debris' }, particleGraph2D.emit('debris-field')),
      particleGraph2D.gate({ kind: 'signal', signal: 'launch-asteroid' }, particleGraph2D.emit('asteroid-stream')),
    ),
  },
