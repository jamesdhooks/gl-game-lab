...
describe('resolveGpuParticleStateSize', () => {
  ...
  it('keeps metadata state opt-in without changing particle packing', () => {
    expect(resolveGpuParticleStateSize({ capacity: 4096, metadata: true })).toEqual({ capacity: 4096, width: 64, height: 64 });
  });
