...
- Production scene modules import ignored build-generated program modules through a stable facade. The generated payload is hydrated through a validating engine boundary that checks compiler/ABI compatibility, graph identity, reflection, shader roles, and every shader source hash before deep-freezing it. Ordinary scene imports no longer compose shaders; development hot reload reruns the same compiler and lets the existing ABI-aware replacement path preserve or reset state.
...
- The WebGPU prototype owns GPU-resident state, compute simulation, priority event queues, bounds/circle/capsule collisions, GPU-owned indirect draw arguments, point/streak rendering through `drawIndirect`, persistent feedback trails, bloom/composite presentation, pipeline caching, and device-loss handling. Its collision path matches the WebGL2 semantic contract for restitution, friction, lifetime loss, kill/bounce modes, and collision-event flags. The development benchmark lab selects it internally on supported hardware and automatically retains or restores WebGL2 presentation when WebGPU initialization or device execution fails.
...
Measured reports are committed under `docs/benchmarks/particle/`. The 36-workload desktop matrix and the emulated mobile preview matrix completed without page errors. The explicit `display-refresh-tolerant-v1` policy keeps the requested 60 FPS target while requiring at least 59 average FPS to tolerate normal display scheduling; GPU p95 budgets are unchanged. All committed desktop and emulated-mobile matrices pass that policy. Physical iOS/Android certification and human scene-parity approval are external acceptance gates, not inferred successes.

The recovery wrapper preserves semantic configuration across a runtime backend failure. Palette, parameters, colliders, force fields, domain, emitter geometry, event tuning, viewport, render parameters, render scale, and diagnostic mode are replayed before the failed operation is retried on WebGL2. Device loss and uncaptured WebGPU validation errors invalidate every resident resource so the next engine operation enters this recovery path. This prevents a successful fallback from silently changing the scene.
