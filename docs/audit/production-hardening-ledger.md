...
| RND-01 | Critical | WebGL context restoration does not recreate owned GPU resources. | 4 | Active | Chromium, Firefox, and WebKit hosted reports pass deterministic invalidation/rebuild with generation/count/byte equivalence. A real embedded-browser driver probe exposed and repaired callback-induced engine remount during loss; the repaired host remains single-mounted without shader/runtime errors but this embedded driver never emits `webglcontextrestored` and reaches the explicit timeout. Physical-browser GPU restoration signoff remains required. |
...
| QLT-01 | High | Large portions of TypeScript are one-line/minified and resist review and maintenance. | 2 | Verified | Formatting baseline is empty; source hygiene and all-package typecheck pass; 71 focused test files are tracked. The repository owns its Vitest discovery so local worktrees cannot inherit legacy parent configuration. |
| QLT-02 | High | Tests are predominantly shallow API tests; real GL, leaks, lifecycle failure, and performance are weakly covered. | 2-9 | Active | Unit/integration/performance coverage, clean CI, and passing three-engine context/touch/gamepad/lifecycle reports exist. Physical mobile performance and real driver-loss cycling remain outstanding evidence. |
