...
| TOL-01 | Medium | Diagnostics, frame/resource inspection, GPU debugging, and capture tooling are incomplete. | 8 | Verified | Live overlay and fixed-frame JSON capture expose per-system CPU, optional non-blocking disjoint GPU timing, draws, points, uploads, GPU bytes, assets, and render passes with tier budget results. |
