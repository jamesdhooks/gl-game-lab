...
The requirement-by-requirement release verdict is recorded in
[`completion-audit-2026-07-11.md`](./completion-audit-2026-07-11.md).
...
| RND-01 | Critical | WebGL context restoration does not recreate owned GPU resources. | 4 | Verified | Hosted Chromium/Firefox/WebKit registry reconstruction passes. Headed physical-host driver cycles pass in installed Chrome 150, installed Edge 150, and Firefox 151: generation advances 0→1, all 6 resources return, 9,280 tracked bytes remain stable, rendering resumes, and no page errors occur. See `physical-driver-matrix-2026-07-11.md`. |
...
| PRF-03 | High | No enforceable desktop/mobile frame-time, memory, draw-call, or upload budgets exist. | 8, 9 | Active | Executable 60/30 FPS budgets cover p95 CPU, draws, uploads, and GPU bytes; all 30 Chromium desktop/mobile-viewport captures and the Chromium/Firefox/WebKit functional matrix pass. The deployed on-device runner serially certifies all 15 profiles and rejects non-iOS-Safari/Android-Chrome evidence. Physical reports remain required. |
...
| WEB-01 | Critical | React teardown ignores asynchronous destroy failures and definition changes can desynchronize host/runtime state. | 5 | Verified | Exact-once async destruction and definition-state reset are unit-tested; production Chromium same-mount replacement proved the old engine destroyed and the new definition reached running. Callback props use live refs instead of reconstruction dependencies, and a real driver-loss probe proved diagnostic rerenders keep one engine mount. |
| WEB-02 | High | Input lacks action maps, gamepad, focus/visibility policy, and complete pointer capture semantics. | 5 | Verified | Input contracts are unit-tested; passing Chromium, Firefox, and WebKit reports record touch down/up observation, pointer counts, and a polled standard gamepad. |
...
| DEM-01 | Critical | The clean rebuild retained a diagnostic host but dropped the established demo gallery and immersive launcher contract. | 5, 6, 9 | Verified | The original React/Tailwind component composition is ported onto the new runtime: HUD, adaptive overflow menu, mode toggle, portal style picker, centered intro card, resolution/settings drawer, floating simulation controls, hide/demo/info actions, live tiles, filters, and responsive three-side navigation. Desktop/mobile shell assertions run in Chromium, Firefox, and WebKit; the same build is deployed to GitHub Pages. |
| QLT-01 | High | Large portions of TypeScript are one-line/minified and resist review and maintenance. | 2 | Verified | Formatting baseline is empty; source hygiene and all-package typecheck pass; 73 focused test files and 234 tests pass. The repository owns its Vitest discovery so local worktrees cannot inherit legacy parent configuration. |
| QLT-02 | High | Tests are predominantly shallow API tests; real GL, leaks, lifecycle failure, and performance are weakly covered. | 2-9 | Active | Unit/integration/performance coverage, clean CI, passing three-engine hosted reports, and real headed Chrome/Edge/Firefox driver cycles exist. Physical mobile performance remains outstanding evidence. |
