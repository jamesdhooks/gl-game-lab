...
| PRF-01 | High | Hot paths allocate temporary objects/arrays and upload avoidable full buffers per frame. | 4, 7, 8 | Verified | Persistent sprite staging removed per-batch backing allocations; tracked transient allocation and upload counters are zero/budgeted across all 30 desktop/mobile-viewport captures. |
...
| QLT-02 | High | Tests are predominantly shallow API tests; real GL, leaks, lifecycle failure, and performance are weakly covered. | 2-9 | Active | Unit/integration tests, production-GL captures, lifecycle/browser probes, and performance gates now exist; forced context restoration and cross-browser/device evidence remain. |
...
| TOL-01 | Medium | Diagnostics, frame/resource inspection, GPU debugging, and capture tooling are incomplete. | 8 | Verified | Live overlay and fixed-frame capture expose CPU/GPU timing, draws, uploads, resources, assets, and passes; dynamic shader compilation adds labeled numbered-source failures and active uniform/attribute reflection. |
