...
| RND-01 | Critical | WebGL context restoration does not recreate owned GPU resources. | 4 | Active | Chromium, Firefox, and WebKit hosted reports pass deterministic invalidation/rebuild with generation/count/byte equivalence. The real `WEBGL_lose_context` driver strategy remains available and requires physical-browser GPU signoff because hosted Linux software drivers suspend page execution after forced loss. |
