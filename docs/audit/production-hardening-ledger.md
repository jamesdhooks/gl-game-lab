...
| RND-01 | Critical | WebGL context restoration does not recreate owned GPU resources. | 4 | Active | Prioritized restoration, built-in renderer-pipeline rebuild, owned texture recreation, and image-source lifetime transfer are implemented and unit-tested; simulation-owned raw resources and browser equivalence proof remain. |
